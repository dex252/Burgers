...
import reducer, { initialState } from './orders-history-slice';
...
describe('orders-history-slice', () => {
	...
	it('Установить новый заказ setOrder', () => {
		...
		expect(
			reducer(initialState, {
				type: 'orders-history-store/setOrder',
				payload: order,
			})
		).toEqual({
			...initialState,
	...
	it('Выполнить _REQUEST', () => {
		expect(
			reducer(initialState, {
				type: 'orders-history-store/_REQUEST',
			})
		).toEqual({
			...initialState,
			order: undefined,
	...
	it('Выполнить _SUCCESS', () => {
		...
		expect(
			reducer(stateWithRequest, {
				type: 'orders-history-store/_SUCCESS',
				payload: order,
			})
		).toEqual({
			...stateWithRequest,
	...
	it('Выполнить _ERROR', () => {
		...
		expect(
			reducer(initialState, {
				type: 'orders-history-store/_ERROR',
	...
	it('Зазорвать соединение wsDisconnect', () => {
		...
		expect(
			reducer(stateWithOrders, {
				type: 'ordersHistory/wsDisconnect',
			})
		).toEqual({
			...stateWithOrders,
			orders: [],
	...
	it('Получить сообщение wsOnMessage', () => {
		...
		expect(
			reducer(stateWithLoading, {
				type: 'ordersHistory/wsOnMessage',
				payload: historyData,
			})
		).toEqual({
			...stateWithLoading,
